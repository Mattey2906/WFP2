...
router.get('/acc_man/unsafe_getUser', validateAndUpdateSession, async (req, res) => {
    try {
        ...
        // UNSICHER: Schritt 1 - User-ID anhand der Session-ID abrufen
        const querySession = `
            SELECT user_id FROM sessions 
            WHERE session_id = '${sessionID}' AND expire_date > NOW()
        `;
        const [sessionResult] = await dbConnection.query(querySession);
        ...
        // UNSICHER: Schritt 2 - Username aus der Users-Tabelle abrufen
        const queryUser = `
            SELECT username FROM users 
            WHERE id = '${userId}'
        `;
        const [userResult] = await dbConnection.query(queryUser);
